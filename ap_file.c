...
void simpan_file_data()	{
	...
	int j, k;
	/*
	isi=pvPortMalloc(128);
	if(isi==NULL){
		printf("Out of Memory SDcard Buffer\r\n");
		vPortFree(isi);
		return;
	...
	* */
	...
	//sprintf(st, "%s\\%02d%02d.str", fol,  ctime0.hours, menit);
	/*
	if (f_mount(&FFS,"0:",1) == FR_OK)	{
		printf("%s(): OK\r\n", __FUNCTION__);
		//st_hw.sdcmounted=1;
	}
	else	{
	...
	*/	
	...
	} else if(hasil==FR_OK) {		// file sudah ada
		res = f_sync(&filx);
		if(d_f==1){
			printf("res sync 1 : %d\r\n",res);
		}
	...
	for (i=0; i<st_file->jml; i++)	{
		...
		st_data = ALMT_DATA + j*JML_KOPI_TEMP;
			...
			if (i==0)	{
				//sprintf(isi, "SN,%s,", st_env->SN);
				f_printf(&filx, "SN,%s,", st_env->SN);
				
				//sprintf(st, "TIME,%ld", now_to_time(1, a));
			...
			sprintf(isi, ",%d,%f", st_data[k].id, data_f[st_file->urut[i]-1]);
			//f_printf(&filx, ",%d,%X", st_data[k].id, data_f[st_file->urut[i]-1]);
			//strcat(isi, st2);
			res=f_puts(isi,&filx);
			...
			//printf("t2 lagi: %s, index: %d\r\n",t2,idx);
			//printf("isinya: %s, datanya: %0.2f, t2: %s\r\n",isi,data_f[idx],t2);
			#else
			if (i==0)	{
				isi[0] = '\r'; isi[1] = '\n';
